...
#define GLOBAL_NAME                                                \
  "global" /* Global coord system name \ \ \ \ \ \ \ \ \ \ \ \ \ \ \
            */
...
--------------------------------*/
#define OBJ_LOC_LABELS " ELT      X1C      X2C      X3C "
#define OBJ_LOC_UNDLNS "---- -------- -------- -------- "
#define OBJ_LOC_FMT "%4d %8.3f %8.3f %8.3f "
...
#define OBJ_STRESS_LABELS "        T1         T2         T3 "
#define OBJ_STRESS_UNDLNS "---------- ---------- ---------- "
#define OBJ_STRESS_FMT "%10.3e %10.3e %10.3e "
#define OBJ_BC_CSYS_LABELS "Coord Sys\n"
#define OBJ_BC_CSYS_UNDLNS "---------\n"
#define OBJ_BC_CSYS_FMT "%s\n"
...
/*********************************************************************************************************/
/************************* NEW: added 98-12-09 to reflect the problem of
 * observation point near vertices */
...
/*       EXPLANATIONS:
    ...
    So coef_exclu_E = 1.0, means 100% of the mean length of segments containing
   v.
*/
/*********************************************************************************************************/
...
void displ_strain(int calc_displ, int calc_strain, double x[3], double displ[3],
                  double strain[3][3], elt_t *omit_elt);
void displ_strain_poly_elt(int calc_displ, int calc_strain, elt_t *current_elt,
                           double x[3], double displ[3], double strain[3][3],
                           elt_t *omit_elt, int under);
void print_obj_data(void);
csys_t *find_csys(const char *name);
vert_t *find_vert(const char *name);
int get_double_var(double *var, const char *var_name, char *word[],
                   int numwords);
int get_boolean_var(int *var, const char *var_name, const char *true_string,
                    const char *false_string, char *word[], int numwords,
                    char *line);
int get_text_var(char **var, const char *var_name, char *word[], int numwords,
                 char *line);
...
[[noreturn]] void p_error(const char *error_msg, const char *line);
void display_msg(char *_msg);
void print_elt_data(obj_t *current_obj, elt_t *current_elt, int elt_num,
                    double displ[3], double stress[3][3]);
...
void print_problem_info(void) {
  ...
  ------------------------------------------*/
  if (print_elt_geom_E) print_elt_geometry();
}
...
void print_obj_data(void) {
  ...
  while (current_elt != NULL) {
    ...
    if (current_obj->print[DISPL] || current_obj->print[STRESS]) {
      ...
      displ_strain(calc_displ, calc_strain, current_elt->elt_csys.origin, displ,
                   strain, current_elt);
      strain_to_stress(strain, shear_mod_E, lame_lambda_E, stress);

      // Print the data for this elt
      print_elt_data(current_obj, current_elt, elt_num, displ, stress);
...
void print_obs_grid_data(void) {
  ...
  while (current_obs_grid != NULL) {
    ...
    calc_strain =
        (print[STRAIN] || print[PSTRAIN] || print[STRESS] || print[PSTRESS]);
    ...
    switch (current_obs_grid->dimension) {
      case 0:
        ...
        transform_position_vector(INVERSE_TRANS,
                                  ...
                                  current_obs_grid->endpt_csys->local_rot, x);
        displ_strain(calc_displ, calc_strain, x, displ, strain, NULL);
          ...
          for (j = 0; j < numpts[1]; j++) {
            ...
            for (k = 0; k < numpts[0]; k++) {
              ...
              transform_position_vector(
                  INVERSE_TRANS, current_obs_grid->endpt_csys->origin,
                  current_obs_grid->endpt_csys->local_rot, x_copy);
              displ_strain(calc_displ, calc_strain, /****/ x_copy /****/, displ,
                           strain, NULL);
              print_obs_pt_data(current_obs_grid, /****/ x_copy /****/, displ,
                                strain);
            }
          }
        }
      ...
      default:
        sprintf(error_msg, "Invalid dimension (%d) for observation grid",
 ...
 *		   x			   - coords (global) of pt at which to
 *calc displ & strain omit_elt	   - element to omit when calculating displs
 *(NULL = none)
 *
...
void displ_strain(int calc_displ, int calc_strain, double x[3], double displ[3],
  ...
  while (current_elt != NULL) {
    ...
    /* Determine if obs_pt is nearnest vertex verta, vertb or vertc
       ...
       2) compute distance from obs_pt to verta and see if this diance is <
       d*coef_exclu 3) compute distance from obs_pt to vertb and see if this
       diance is < d*coef_exclu 4) compute distance from obs_pt to vertc and see
       if this diance is < d*coef_exclu if condition is TRUE, break the loop
       over elements and set the flag near_vertex_E=TRUE and return.
    ...
    */
    ...
    if (distance(x, verta->x) <= dist || distance(x, vertb->x) <= dist ||
        distance(x, vertc->x) <= dist) {
      near_vertex_E = TRUE;
      break;
    }
    ...
    --------------------------------------------------------*/
    displ_strain_poly_elt(calc_displ, calc_strain, current_elt, x, elt_displ,
                          elt_strain, omit_elt, under);
  ...
  ----------------------------------*/
  if (near_vertex_E != TRUE) add_matrices(strain, rem_strain_E, strain);
}
 ...
 *		x			- coords (global) of pt at which to calc
 *displ & strain omit_elt	- element to omit when calculating displs (NULL
 *= none) Out:	elt_displ  	- displacement vector (global coords) due to
 *this elt elt_strain	- strain tensor (global coords) due to this elt
 ****************************************************************************/
void displ_strain_poly_elt(int calc_displ, int calc_strain, elt_t *current_elt,
                           double x[3], double elt_displ[3],
                           double elt_strain[3][3], elt_t *omit_elt,
                           int under) {
  double displ_ic[3][3];
  ...
  ---------------------------------------------------------------*/
  displ_strain_ics_poly_elt(calc_displ, calc_strain, current_elt, x, displ_ic,
                            strain_ic, omit_elt);
 ...
 ---------------------------------------------------------------------------------*/
  ...
  if (under > 0) {
    ...
    */
    ...
    for (i = 0; i < 3; i++) {
      for (j = 0; j < 3; j++) bglobal[i] += dot_product(bg[j], eg[i]);

      /*Corrects the displacement by the corresponding burger's vector (global
      coordsys)
      --------------------------------------------------------------------------------*/
      if (under == 1)
 ...
 *                        	  (bc_coord_sys coords)
 *		strain_ic[i][j][k]	- the jk component of strain (global
 *coords) due to a unit ith Burgers vector component (bc_coord_sys coords)
 ****************************************************************************/
void displ_strain_ics_poly_elt(int calc_displ, int calc_strain,
                               elt_t *current_elt, double x[3],
                               double displ_ic[3][3], double strain_ic[3][3][3],
...
for (i=0; i<=num_eqns-1; i++){
    ...
    if (calc_displ) {
      ...
      if (current_elt != omit_elt) {
        comninou_displ_ics(y1, depth1, beta, psn_ratio_E, half_space_E,
                           displ_ic1);
        comninou_displ_ics(y2, depth2, beta, psn_ratio_E, half_space_E,
                           displ_ic2);

        /* Superpose the angular dislocation influence coeffs into
        a dislocation segment influence coeff
        ----------------------------------------------------------*/
        subtract_matrices(displ_ic1, displ_ic2, displ_ic3);

        /* Swap the vertices back to proper order (if necessary)
        --------------------------------------------------------*/
        if (swap) {
          scalar_vector_mult(-1.0, displ_ic3[2], displ_ic3[2]);
          for (i = 0; i < 3; i++) {
            displ_ic3[i][0] *= -1.0;
            displ_ic3[i][1] *= -1.0;
          }
        ...
        for (i = 0; i < 3; i++) {
          ...
          for (j = 0; j < 3; j++) {
            for (k = 0; k < 3; k++) {
              displ_ic4[i][j] += disloc_seg[seg].elt_b[i][k] * displ_ic3[k][j];
            }
          }
        }
        ...
        ----------------------------------------*/
        for (i = 0; i < 3; i++) {
          rotate_vector(INVERSE_ROT, disloc_seg[seg].local_rot, displ_ic4[i]);
        }

        /* Superpose the contribution of this dislocation segment
        ---------------------------------------------------------*/
        for (i = 0; i < 3; i++) {
          add_vectors(displ_ic[i], displ_ic4[i], displ_ic[i]);
        }
      }
    }
    ...
    ------------------------------------*/
    if (calc_strain) {
      comninou_strain_ics(y1, depth1, beta, psn_ratio_E, half_space_E,
                          strain_ic1);
      comninou_strain_ics(y2, depth2, beta, psn_ratio_E, half_space_E,
                          strain_ic2);
      ...
      ----------------------------------------------------------*/
      for (i = 0; i < 3; i++) {
        subtract_matrices(strain_ic1[i], strain_ic2[i], strain_ic3[i]);
      }

      /* Swap the vertices back to proper order (if necessary)
      --------------------------------------------------------*/
      if (swap) {
        scalar_matrix_mult(-1.0, strain_ic3[2], strain_ic3[2]);
        for (i = 0; i < 3; i++) {
          strain_ic3[i][0][2] *= -1;
          strain_ic3[i][2][0] *= -1;
          strain_ic3[i][1][2] *= -1;
      ...
      for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < 3; j++) {
          for (k = 0; k < 3; k++) {
            for (l = 0; l < 3; l++) {
              strain_ic4[i][j][k] +=
                  disloc_seg[seg].elt_b[i][l] * strain_ic3[l][j][k];
            }
          }
        }
      }
      ...
      ---------------------------------------------------------*/
      for (i = 0; i < 3; i++) {
        add_matrices(strain_ic[i], strain_ic4[i], strain_ic[i]);
      }
    }
...
csys_t *find_csys(const char *name) {
  ...
  while (current_csys != NULL) {
    if (!strcmp(name, current_csys->name)) break;
...
vert_t *find_vert(const char *name) {
  ...
  while (current_vert != NULL) {
    if (!strcmp(name, current_vert->name)) break;
 ...
 * 		line		- input file line number at which the error
 *occurred (NULL = N/A)
 *****************************************************************************/
 ...
 * In:	error_msg	- error message to print
 *****************************************************************************/
void display_msg(char *_msg) { fprintf(stderr, "%s\n", _msg); }
...
void setup_global_coords(void) {
  ...
  -----------------------------------------------*/
  first_csys_E = (csys_t *)calloc((size_t)1, sizeof(csys_t));
  if (!first_csys_E)
    p_error("Cannot allocate memory (calloc) for global coord sys", NULL);

  first_csys_E->name = (char *)malloc((size_t)strlen((GLOBAL_NAME) + 1));
  if (!first_csys_E->name)
    p_error("Cannot allocate memory for global coord system name", NULL);
...
int read_constants(void) {
  ...
  for (;;) {
    ...
    -------------------------------------*/
    if (!strcmp(word[0], END_STMT)) break;

    /* parse constants
    ------------------*/

    if (get_text_var(&title1_E, "title1", word, numwords, line)) continue;

    if (get_text_var(&title2_E, "title2", word, numwords, line)) continue;

    if (get_double_var(&shear_mod_E, "shear_mod", word, numwords)) continue;

    if (get_double_var(&psn_ratio_E, "psn_ratio", word, numwords)) continue;

    if (get_double_var(&youngs_mod_E, "youngs_mod", word, numwords)) continue;

    if (get_double_var(&bulk_mod_E, "bulk_mod", word, numwords)) continue;

    if (get_double_var(&lame_lambda_E, "lame_lambda", word, numwords)) continue;

    if (get_double_var(&null_value_E, "null_value", word, numwords)) continue;
    /*********************************************** ADDED 98-12-09 */
    if (get_double_var(&coef_exclu_E, "coef_exclu", word, numwords)) continue;
    /***************************************************************/

    if (get_boolean_var(&rem_stress_bc_E, "rem_bc_type", "stress", "strain",
                        word, numwords, line))
      continue;

    if (get_double_var(&s11r, "s11r", word, numwords)) continue;

    if (get_double_var(&s22r, "s22r", word, numwords)) continue;

    if (get_double_var(&s33r, "s33r", word, numwords)) continue;

    if (get_double_var(&s12r, "s12r", word, numwords)) continue;

    if (get_double_var(&s13r, "s13r", word, numwords)) continue;

    if (get_double_var(&s23r, "s23r", word, numwords)) continue;

    if (get_boolean_var(&half_space_E, "half_space", "yes", "no", word,
                        numwords, line))
      continue;

    if (get_boolean_var(&check_cond_num_E, "check_cond_num", "yes", "no", word,
                        numwords, line))
      continue;

    if (get_boolean_var(&print_elt_geom_E, "print_elt_geom", "yes", "no", word,
                        numwords, line))
      continue;

    if (get_text_var(&elt_geom_csys_name_E, "elt_geom_csys", word, numwords,
                     line))
      continue;
  ...
  rem_stress_E[0][1] = s12r;
  rem_stress_E[1][0] = s12r;
  rem_stress_E[0][2] = s13r;
  rem_stress_E[2][0] = s13r;
  rem_stress_E[1][2] = s23r;
  rem_stress_E[1][2] = s23r;
  copy_matrix(rem_stress_E, rem_strain_E);
  if (rem_stress_bc_E) {
    stress_to_strain(rem_strain_E, youngs_mod_E, psn_ratio_E, rem_strain_E);
  } else {
    strain_to_stress(rem_stress_E, shear_mod_E, lame_lambda_E, rem_stress_E);
  }
...
int read_csystems(void) {
  ...
  for (;;) {
    ...
    ----------------------------------------*/
      ...
      sprintf(error_msg, "Too %s parameters specified to define coord system",
    ...
    -------------------------------------------------------------*/
    current_csys->next = (csys_t *)calloc((size_t)1, sizeof(csys_t));
    if (!current_csys->next)
      p_error("Cannot allocate memory (calloc) for local coord sys", line);
    current_csys = current_csys->next;

    /* Assign coordinate system name
    --------------------------------*/
    current_csys->name = (char *)malloc((size_t)strlen(word[CS_NAME_POS]) + 1);
    if (!current_csys->name)
      p_error("Cannot allocate memory for coord system name", line);
    strcpy(current_csys->name, word[CS_NAME_POS]);

    /* Get coordinate system parent
    -------------------------------*/
    if ((parent = find_csys(word[CS_PARENT_POS])) == NULL) {
    ...
    transform_position_vector(INVERSE_TRANS, parent->origin, parent->local_rot,
                              current_csys->origin);
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (temp_char < '1' || temp_char > '3') {
        p_error("Invalid axis for rotation order", line);
      }
    ...
    for (i = 0; i < 3; i++) {
      ...
      rot_matrix[i][j][j] = rot_matrix[i][k][k] = cos(rot[i]);
      rot_matrix[i][k][j] = -(rot_matrix[i][j][k] = sin(rot[i]));
    }
  ...
  for (;;) {
    ...
    switch (dimension) {
      ...
      case 3:
        ...
        break;
      default:
        p_error("Invalid dimension for observation grid", line);
    }

    if (!correct_num_params) {
      sprintf(error_msg, "Incorrect number of parameters for %d-D grid",
              dimension);
      p_error(error_msg, line);
    }
    ...
    if (first_obs_grid_E == NULL) {
      first_obs_grid_E = (obs_grid_t *)calloc((size_t)1, sizeof(obs_grid_t));
      if (!first_obs_grid_E)
        p_error("Cannot allocate memory (calloc) for obs grid", line);
      current_obs_grid = first_obs_grid_E;
    } else {
      current_obs_grid->next =
          (obs_grid_t *)calloc((size_t)1, sizeof(obs_grid_t));
    ...
    ----------------------------*/
    current_obs_grid->name =
        (char *)malloc((size_t)strlen(word[OG_NAME_POS]) + 1);
    if (!current_obs_grid->name)
      p_error("Cannot allocate memory for observation grid name", line);
    ...
    ----------------------------------*/
    if ((current_obs_grid->endpt_csys = find_csys(word[OG_INPUT_CSYS_POS])) ==
        NULL) {
      p_error("Undefined coordinate system", line);
    }
    ...
    ----------------------------------------------*/
    if ((current_obs_grid->obspt_csys = find_csys(word[OG_OBSPT_CSYS_POS])) ==
        NULL) {
      p_error("Undefined coordinate system", line);
    }
    ...
    -----------------------------------*/
    if ((current_obs_grid->outp_csys = find_csys(word[OG_DATA_CSYS_POS])) ==
        NULL) {
      p_error("Undefined coordinate system", line);
    }
    ...
    for (i = 0; i < ((dimension == 2) ? 3 : dimension); i++) {
      current_obs_grid->numpts[i] = numpts = atoi(word[OG_NUMPTS_POS + i]);
      if (numpts < 2) {
    ...
    if (dimension == 2 && num_ones != 1) {
      p_error("1 (& only 1) 2D observation grid axis requires 1 point", line);
    }
...
int read_objs_elts_verts(void) {
  ...
  for (;;) {
    ...
    --------------------*/
      ...
      get_elt_info(&current_elt, current_obj, numwords, word, line);
    }
  }
 ...
 *
 * In:	current_obs_grid	- the obs grid for which titles are to be
 *printed
 *****************************************************************************/
void print_obs_grid_titles(obs_grid_t *current_obs_grid) {
  fprintf(ofp_E, "\n\n====================================================\n");
 ...
 * Out:	stress		- the stress tensor at the element center due to ALL
 *elts displ		- the displacement of the element center due to ALL elts
 *					  EXCEPT this element
 ****************************************************************************/
void print_elt_data(obj_t *current_obj, elt_t *current_elt, int elt_num,
                    double displ[3], double stress[3][3]) {
  ...
  transform_position_vector(FORWARD_TRANS, current_obj->pos_csys->origin,
                            current_obj->pos_csys->local_rot, center);

  /* Print element location info
  ------------------------------*/
  for (i = 0; i < NUM_PR_OPTS; i++) {
    if (current_obj->print[i]) {
      fprintf(tempfp_E[i], OBJ_LOC_FMT, elt_num, center[0], center[1],
              center[2]);
    }
  }
  ...
  if (current_obj->print[DISPL]) {
    ...
    fprintf(tempfp_E[DISPL], OBJ_DISPL_FMT, b[0], displ_pos[0], displ_neg[0],
            b[1], displ_pos[1], displ_neg[1], b[2], displ_pos[2], displ_neg[2]);
  }
  ...
  if (current_obj->print[STRESS]) {
    ...
    normal_vector[2] = -1.0;
    rotate_vector(INVERSE_ROT, current_elt->elt_csys.local_rot, normal_vector);
    cauchy(stress, normal_vector, traction);
    rotate_vector(FORWARD_ROT, current_elt->bc_csys->local_rot, traction);

    fprintf(tempfp_E[STRESS], OBJ_STRESS_FMT, traction[0], traction[1],
            traction[2]);
  }
  ...
  for (i = 0; i < NUM_PR_OPTS; i++) {
    if (current_obj->print[i]) {
      fprintf(tempfp_E[i], OBJ_BC_CSYS_FMT, current_elt->bc_csys->name);
    }
  }
}
 ...
 *of the observation point displ				- the
 *displacement
 *(global coords) at the obs pt strain				- the strain
 *(global coords) at the obs point
 ****************************************************************************/
void print_obs_pt_data(obs_grid_t *current_obs_grid, double x[3],
                       double displ[3], double strain[3][3]) {
  double stress[3][3];
  ...
  if (current_obs_grid->print[DISPL]) {
    ...
    if (below_vertex_E || near_vertex_E) initialize_vector(displ, null_value_E);
  ...
  if (current_obs_grid->print[STRESS] || current_obs_grid->print[PSTRESS]) {
    ...
    if (current_obs_grid->print[PSTRESS]) {
      ...
      fprintf(tempfp_E[PSTRESS], OG_PSTRESS_FMT, traj[0][0], traj[0][1],
              traj[0][2], prin[0], traj[1][0], traj[1][1], traj[1][2], prin[1],
  ...
  if (current_obs_grid->print[STRAIN] || current_obs_grid->print[PSTRAIN]) {
    ...
    if (current_obs_grid->print[PSTRAIN]) {
      ...
      fprintf(tempfp_E[PSTRAIN], OG_PSTRAIN_FMT, traj[0][0], traj[0][1],
              traj[0][2], prin[0], traj[1][0], traj[1][1], traj[1][2], prin[1],
              traj[2][0], traj[2][1], traj[2][2], prin[2]);
    }
  }
...
void determine_burgers_vectors() {
  ...
  while (elt1 != NULL) {
    for (i = 0; i < 3; i++) {
      if (elt1->bc_type[i] == TRACTION_BC) num_eqns++;
    }
    elt1 = elt1->next;
  }
  ...
  for (i = 1; i <= num_eqns; i++) fprintf(outVec, "%f\n", b_vector[i]);
  fprintf(outVec, "\n");
#endif
  ...
  while (elt1 != NULL) {
    index++;
    if (elt1->bc_type[0] == TRACTION_BC || elt1->bc_type[1] == TRACTION_BC ||
      ...
      while (elt2 != NULL) {
        ...
        if (below_vertex_E)
          p_error("One elt's center lies directly under another's vertex",
                  NULL);
        for (i = 0; i < 3; i++) {
        ...
        for (i = 0; i < 3; i++) {
          ...
          rotate_vector(FORWARD_ROT, elt1->bc_csys->local_rot, traction_ic[i]);
        }
          ...
          if (elt1->bc_type[i] == TRACTION_BC) {
            ...
            for (j = 0; j < 3; j++) {
              switch (elt2->bc_type[j]) {
                case BVECTOR_BC:
                  b_vector[row + rplus] -= traction_ic[j][i] * (*elt2->b[j]);
                  break;
                case TRACTION_BC:
                  ic_matrix[row + rplus][col + cplus] = traction_ic[j][i];
                  cplus++;
                  break;
              } /*switch*/
            }   /*for*/
  ...
  for (i = 1; i <= num_eqns; i++) {
    for (j = 1; j <= num_eqns; j++) fprintf(outMat, "%f\t", ic_matrix[j][i]);
    fprintf(outMat, "\n");
  }
  for (i = 1; i <= num_eqns; i++) fprintf(outVec, "%f\n", b_vector[i]);
#endif
  ...
  if (check_cond_num_E && num_eqns != 0) {
    ...
    -----------------------------*/
    for (i = 1; i <= num_eqns; i++) {
      for (j = 1; j <= num_eqns; j++) column[j] = 0.0;
      column[i] = 1.0;
      d_lubksb(ic_matrix, num_eqns, pivot, column);
      for (j = 1; j <= num_eqns; j++) ic_matrix_inv[j][i] = column[j];
    }
...
int read_line(char *line, char *word[]) {
  ...
  ---------------------------------*/
  if ((numwords =
           getwords(ifp_E, line, MAXLINE, word, MAXWORDS, CONTINUE_CHAR)) < 0) {
    switch (numwords) {
      case GW_EOF_ERR:
 ...
 * In/Out:	var			- variable to which a value is to be
 *assigned
 *****************************************************************************/
int get_double_var(double *var, const char *var_name, char *word[],
                   int numwords) {
 ...
 * assignment is made, or FALSE otherwise.
 *
 * In:		var_name		- name used for this variable in the
 *input file true_string		- true value string for this var in
 *input file false_string	- false value string for this var in input file
 *word			- array of words given in the input line numwords
 *- number of words on the input line line			- line read from
 *the input file In/Out: var
    ...
    if (numwords == CONST_NUM_PARAMS) {
      if (!strcmp(word[CONST_VALUE_POS], true_string))
        ...
        *var = FALSE;
        ...
        sprintf(error_msg, "%s requires a value of \"%s\" or \"%s\"", var_name,
 ...
 *			word			- array of words given in the
 *input line line			- the input line In/Out:
 *current_vert
 *- the current (last) vertex in the linked list
 ****************************************************************************/
void get_vert_info(vert_t **current_vert, int numwords, char *word[],
                   char *line) {
  int i;
  ...
  if (numwords != V_NUM_PARAMS) {
    sprintf(error_msg, "Too %s parameters specified to define vertex",
  ...
  if (first_vert_E == NULL) {
    ...
    if (!first_vert_E)
      p_error("Cannot allocate memory (calloc) for vertex", line);
    *current_vert = first_vert_E;
  } else {
    (*current_vert)->next = (vert_t *)calloc((size_t)1, sizeof(vert_t));
  ...
  /* Get vertex name
  ------------------*/
  (*current_vert)->name = (char *)malloc((size_t)strlen(word[V_NAME_POS]) + 1);
  if (!(*current_vert)->name)
    p_error("Cannot allocate memory for vertex name", line);
  strcpy((*current_vert)->name, word[V_NAME_POS]);

  /* Get the coodinate system
  ----------------------------*/
  if (((*current_vert)->csys = find_csys(word[V_CSYS_POS])) == NULL) {
    p_error("Undefined coordinate system", line);
  }

  /* Read the vertex position
  ---------------------------*/
  for (i = 0; i < 3; i++) (*current_vert)->x[i] = atof(word[V_X_POS + i]);

  /* Transform vertex position vector to global coords
  ----------------------------------------------------*/
  transform_position_vector(INVERSE_TRANS, (*current_vert)->csys->origin,
 ...
 *input line line			- the input line In/Out:
 *current_obj
 *- the current (last) object in the linked list
 *****************************************************************************/
void get_obj_info(obj_t **current_obj, int numwords, char *word[], char *line) {
  int i;
  char temp_char;
  ...
  /* Check for proper number of parameters
  ----------------------------------------*/
  if (numwords != OBJ_MIN_NUM_PARAMS && numwords != OBJ_MIN_NUM_PARAMS + 2) {
    p_error("Incorrect number of parameters specified to define object", line);
  }
  ...
  if (first_obj_E == NULL) {
    ...
    if (!first_obj_E)
      p_error("Cannot allocate memory (calloc) for object", line);
    *current_obj = first_obj_E;
  } else {
  ...
  /* Get object name
  ------------------*/
  (*current_obj)->name = (char *)malloc((size_t)strlen(word[OBJ_NAME_POS]) + 1);
  if (!(*current_obj)->name)
    p_error("Cannot allocate memory for object name", line);
  ...
  if (numwords > OBJ_MIN_NUM_PARAMS) {
    ...
    ------------------------*/
    i = 0;
    (*current_obj)->print[DISPL] = (*current_obj)->print[STRESS] = FALSE;
    ...
    --------------------------------------------------*/
    if (((*current_obj)->pos_csys = find_csys(word[OBJ_POS_CSYS_POS])) ==
        NULL) {
      p_error("Undefined coordinate system", line);
    }
  }
 ...
 *input line line			- the input line In/Out:
 *current_elt
 *- the current (last) element in the linked list current_obj		- the
 *object to which current_elt belongs
 *****************************************************************************/
void get_elt_info(elt_t **current_elt, obj_t *current_obj, int numwords,
                  char *word[], char *line) {
  int num_vertices;
  ...
  if (numwords != num_params) {
    sprintf(error_msg, "Too %s parameters specified to define %d-sided element",
  ...
  if (first_elt_E == NULL) {
    ...
    if (!first_elt_E) p_error("Cannot allocate memory for element", line);
    *current_elt = first_elt_E;
  ...
  if (current_obj->first_elt == NULL) current_obj->first_elt = *current_elt;
  ...
  ------------------------------------------------*/
  (*current_elt)->disloc_seg =
      (disloc_seg_t *)calloc((size_t)num_vertices, sizeof(disloc_seg_t));
  if (!(*current_elt)->disloc_seg)
    p_error("Cannot allocate memory for dislocation segment array", line);
  ...
  if (elt_csys_name == NULL) {
    ...
    if (!elt_csys_name)
      p_error("Cannot allocate memory for elt-local coord sys name", NULL);
  ...
  if (vector_magnitude(x[2]) < TINY_ANGLE)
    p_error("Cannot calc element normal. Elt must have a very odd shape.",
            line);
  normalize_vector(x[2]);
  cross_product(global_x[2], x[2], x[1]);
  if (vector_magnitude(x[1]) < TINY_ANGLE) copy_vector(global_x[1], x[1]);
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
      (*current_elt)->elt_csys.local_rot[i][j] = dot_product(x[i], global_x[j]);
    }
  }
  ...
  for (seg = 0; seg < num_vertices; seg++) {
    ...
    transform_position_vector(FORWARD_TRANS, (*current_elt)->elt_csys.origin,
                              (*current_elt)->elt_csys.local_rot, vert_x);
    if (fabs(vert_x[2]) >
        fabs(sqrt(vert_x[0] * vert_x[0] + vert_x[1] * vert_x[1]) /
             COPLANAR_LIMIT))
  ...
  } else if (((*current_elt)->bc_csys = find_csys(word[E_BC_CSYS_POS])) ==
             NULL) {
    p_error("Undefined coordinate system", line);
  }
  ...
  rotate_vector(FORWARD_ROT, (*current_elt)->bc_csys->local_rot,
                trac_bc_adjust);
  ...
  for (seg = 0; seg < (*current_elt)->num_vertices; seg++) {
    for (i = 0; i < 3; i++) {
      ...
      rotate_vector(INVERSE_ROT, (*current_elt)->bc_csys->local_rot,
...
void copy_temp_files(void) {
  ...
  for (i = 0; i < NUM_PR_OPTS; i++) {
    if ((tfp = tempfp_E[i]) != NULL) {
      rewind(tfp);
      while ((c = fgetc(tfp)) != EOF) fputc(c, ofp_E);
    }
  }
}
...
void print_elt_geometry(void) {
  ...
  if (elt_geom_csys_name_E == NULL) {
    if ((elt_geom_csys = find_csys(GLOBAL_NAME)) == NULL) {
      p_error("Cannot find default coordinate system for elt_geom_csys", NULL);
    }
  } else {
    if ((elt_geom_csys = find_csys(elt_geom_csys_name_E)) == NULL) {
      p_error("Coord sys given for elt_geom_csys was never defined", NULL);
    }
  }
  ...
  while (current_elt != NULL) {
    ...
    for (i = 0; i < current_elt->num_vertices; i++) {
      ...
      transform_position_vector(FORWARD_TRANS, elt_geom_csys->origin,
                                elt_geom_csys->local_rot, x);
      if (i == 0) copy_vector(x, first_vert_x);
    ...
    if (current_elt == current_obj->last_elt) current_obj = current_obj->next;
