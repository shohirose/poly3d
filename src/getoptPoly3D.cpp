...
struct arg_rec *read_args(int argc, char *argv[], const char *arg_string);
 ...
 ****************************************************************************/
int getoptPoly3D(const char *arg_string, int argc, char *argv[]) {
  ...
  if (!next_arg) return (NO_MORE_ARGS);
...
struct arg_rec *read_args(int argc, char *argv[], const char *arg_string) {
  ...
  while (i < argc) {
    if (argv[i][0] == '-') {
      ...
      if (!c_ptr) return ((struct arg_rec *)NO_SUCH_ARG);
      if (*(c_ptr + 1) == ':') {
        add_arg(&top, i, FLAG_AND_FILE, argv);
        i += 2;
      } else
        add_arg(&top, i++, FLAG, argv);
    } else
      add_arg(&top, i++, FILE_ARG, argv);
  }
  return (top);
}
