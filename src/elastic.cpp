...
int calc_elas_consts(elas_const_t *sm, elas_const_t *pr, elas_const_t *ym,
  ...
  num_defined = 0;
  if
    EC_DEFINED(*sm) num_defined++;
  if
    EC_DEFINED(*pr) num_defined++;
  if
    EC_DEFINED(*ym) num_defined++;
  if
    EC_DEFINED(*bm) num_defined++;
  if
    EC_DEFINED(*ll) num_defined++;
  ...
  --------------------------------------------------------------------*/
  if (!EC_DEFINED(*ym))
    (*ym) = (3. * (*ll) + 2. * (*sm)) / ((*ll) + (*sm)) * (*sm);
  if (!EC_DEFINED(*pr)) (*pr) = (*ll) / (2. * ((*ll) + (*sm)));
  if (!EC_DEFINED(*bm)) (*bm) = (*ll) + 2. / 3. * (*sm);
 ...
 ****************************************************************************/
void strain_to_stress(double strain[3][3], double shear_mod, double lame_lambda,
                      double stress[3][3]) {
  int i, j;
  ...
  for (i = 0; i < 3; i++) {
    stress[i][i] =
        lame_lambda * vol_strain + 2. * shear_mod * strain_copy[i][i];
    for (j = 0; j < 3; j++) {
      if (i != j) {
 ...
 ****************************************************************************/
void stress_to_strain(double stress[3][3], double youngs_mod, double psn_ratio,
                      double strain[3][3]) {
  int i, j, k;
  ...
  for (i = 0; i < 3; i++) {
    ...
    k = i + 2;
    if (k > 2) k -= 3;
    strain[i][i] = (stress_copy[i][i] -
                    psn_ratio * (stress_copy[j][j] + stress_copy[k][k])) /
                   youngs_mod;
    for (j = 0; j < 3; j++) {
      if (i != j) {
