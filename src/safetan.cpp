...
safetan_t safe_tan(safetan_t alpha) {
  ...
  if ((half_pi - fabs(alpha)) < SAFE_TAN_LIMIT) {
    alpha =
        (alpha > 0) ? half_pi - SAFE_TAN_LIMIT : -(half_pi - SAFE_TAN_LIMIT);
  }
 ...
 **************************************************************************/
safetan_t safe_atan(safetan_t y, safetan_t x) {
 ...
 *****************************************************************************/
safetan_t safe_atan2(safetan_t y, safetan_t x) {
  /* Avoid huge y/x ratios
  ------------------------*/
  if (fabs(x * SAFE_ATAN_LIMIT) < fabs(y))
  ...
  /* Avoid atan2 domain error for x=y=0
  -------------------------------------*/
  if (y == 0.0 && x == 0.0) return ((x >= 0.0) ? 0.0 : pi);
